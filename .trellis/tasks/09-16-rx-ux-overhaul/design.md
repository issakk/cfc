...
## 13. 第三轮：把「用什么分辨率扫」交给用户

### 13.1 为什么要有这个开关

实测结论：分辨率调高后**明显更快**（用户实测），原因是每个 code cell 在相机里分到的源像素更多 → 每帧读得更准。但更贵的是 CPU 与帧率，最优点取决于机器/距离/模式，代码里推不出来，只能量。所以把选择权交出去，并把代价显示出来（fps / kB/s / scan / chunk / drop）。

### 13.2 选项设计

| 选项 | 取值 | 说明 |
|---|---|---|
| Auto | 短边在 960–1080 档里**最小**的（通常 1280x960） | 上游原行为，帧率优先 |
| Sharp | 同档里**最大**的（通常 1920x1080） | 每 cell 像素更多 |
| 具体尺寸 | 设备上报的任意尺寸（≥720p，按像素从大到小列出） | 可到 1440p/4K，代价由指标显示 |

被选中的具体尺寸若设备不再上报（换机、列表过期），自动回落到档位逻辑，因此坏选择不会让相机起不来。列表来自 `OpencvCameraView` 在开相机时记录的 `params.getSupportedPreviewSizes()`（相机线程写、UI 线程读，用 `synchronized` 保护）。

### 13.3 UI 放哪：点那行小字

预览尺寸属于「改变拍摄参数」，不是「每次都能扫到」的常用操作，所以不占按钮区（右下角三个按钮已经是上限：Send / Mode / Inbox）。

- **入口**：状态条下面那行小字（它本来就显示 `preview 1920x1080 · 27 fps · 48 kB/s · ...`）——点它打开选择器。入口就在「显示结果的地方」，不需要额外解释。
- **对话框**：复用与模式选择器相同的 `showChoices()`（`ListView` 交给 `setView`；`setSingleChoiceItems` 在这个主题下渲染不出来），当前项打 `> `。
- **生效**：`disableView()/enableView()` 把相机重开一次（尺寸只在开相机时决定），那行小字随之刷新为新尺寸。

后续如果要做点按对焦/手电筒/变焦（本轮的非目标），它们和预览尺寸同属「拍摄参数」，应该共用这个入口——届时那行小字可以升级成一个「相机」按钮（把 fps/kB/s 挪进对话框里），而不是继续往按钮区堆。
